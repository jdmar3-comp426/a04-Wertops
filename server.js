...
app.post("/app/new/", function (req, res){
	const stmt = res.send("INSERT INTO userinfo (user, pass) VALUES (?, ?)");
	res.status(201).json(stmt);//not sure if above line should be defined like that
	//anything else?
})
...
app.patch("/app/update/user/:id", function(req, res){
	...
	res.status(200).json(stmt); //check this whole patch thing again later
})
